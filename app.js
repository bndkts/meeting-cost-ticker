...
function countCost() {
  counter = window.setInterval(function () {
    ...
    count += totalCosts;
    value.textContent = inputCurrency.value + count.toFixed(2);
  }, 1000);
}

function startCount() {
  document.getElementById("cost-output").innerHTML = inputCurrency.value + inputCost.value;
...
function resetCount() {
  window.clearInterval(counter);
  count = 0;
  value.textContent = count + inputCurrency.value;
